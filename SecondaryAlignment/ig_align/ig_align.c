...
static aln_t align_read_against_one(kseq_t *target, const int read_len,
                                    ...
                                    const int min_score, int temp, int temp2) {
  uint8_t *ref_num = calloc(target->seq.l, sizeof(uint8_t));
  for (size_t k = 0; k < target->seq.l; ++k)
    ref_num[k] = conf->table[(int)target->seq.s[k]];
  FILE *write_truncate;
  write_truncate = fopen("output_truncate.txt", "a");
  // fprintf(write_truncate, "\nSequence %i: ", temp);
  // for (int i = 0; i < read_len; i++)
  //   fprintf(write_truncate, "%hhu", read_num[i]);
  if (temp == 2 && temp2 == 0) {
    for (int i = 0; i < read_len; i++) {
      fprintf(write_truncate, "%hhu", read_num[i]);
    }
...
static aln_v align_read(const kseq_t *read, const kseq_v targets,
  ...
  for (size_t j = 0; j < kv_size(targets); j++) {
    // Encode target
    r = &kv_A(targets, j);
    aln_t aln = align_read_against_one(r, read_len, read_num, &qry, conf,
                                       min_score, 0, 1);
    if (aln.cigar != NULL) {
